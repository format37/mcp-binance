...
echo "🐳 Starting Binance MCP Server with Docker Compose (Local Mode)..."
...
docker rmi -f mcp-binance-mcp-binance 2>/dev/null || true

# Build and start services
echo "🏗️  Building and starting services..."
docker compose -f docker-compose.local.yml up -d --build --remove-orphans

echo "✅ Binance MCP Server is running in local mode!"
echo "🔧 MCP Endpoint: http://localhost:8010/binance/"
echo "🏥 Health Check: http://localhost:8010/health"
echo ""
echo "For reverse proxy configuration, add to your Caddyfile:"
echo "    handle_path /binance/* {"
echo "        reverse_proxy mcp-binance-local:8010"
echo "    }"
echo ""
echo "To check logs:"
